...
if os.environ.get("ENABLE_PHOENIX","FALSE") == "TRUE":
    ...
    #from eventstream import consume_events

    PHOENIX_ENDPOINT = os.environ.get("PHOENIX_ENDPOINT", "https://phoenix-otel.xoe.xfinity.com/v1/traces")

    # Configure the Phoenix tracer
    tracer_provider = register(
        project_name="DEP",  # Default is 'default'
...
#from ws_con_mngr import WebSocketConnectionManager
...
def get_fast_api_app(
  ...
  @app.post("/apps/{app_name}/users/{user_id}/sessions/{session_id}/audio/transcribe",response_model_exclude_none=True,)
  async def transcribe_audio(app_name: str, user_id: str, session_id: str,request_body:dict):

    AZURE_SPEECH_KEY = os.getenv("AZURE_SPEECH_KEY")
    AZURE_REGION = os.getenv("AZURE_REGION")

    try:
      pcm_bytes_base64 = request_body.get("pcm_bytes")
      pcm_bytes = base64.b64decode(pcm_bytes_base64)
      # Azure Speech Config
      ...
      if result.reason == speechsdk.ResultReason.RecognizedSpeech:
          text = result.text
          print("[AZURE] Transcribed:", text)
      else:
          text = "Could not recognize speech."
          print("[AZURE] NoMatch or Error:", result.reason)
          return { "text": None, "success":False, "error": str(result.reason) }

      # Send text
      return { "text": text, "success":True }
    except Exception as e:
      logger.error("Error during Azure transcription: %s", e)
  ...
  async def agent_run_sse(req: AgentRunRequest) -> StreamingResponse:
    ...
    session.state["userId"] = req.user_id
    if not session:
      raise HTTPException(status_code=404, detail="Session not found")
  ...
  if web:
    ...
    app.mount(
        "/dev-ui/",
        StaticFiles(directory="./argus_web/dist/agent_framework_web/browser", html=True),
        name="static",
    )
  return app
...
if __name__ == "__main__":
  ...
  uvicorn.run(app, host="0.0.0.0", port=8098, log_level="debug")